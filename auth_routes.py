...
def signup():
    ...
    try:
        ...
        send_mail(email, "Verify your SocioChat account", email_body)
        email_sent = True
    except Exception:
        logger.exception("Failed to send verification email")

    # 2. Also send SAME code via SMS if phone provided
    if phone:
        try:
            ...
            success, resp = send_sms_horizon(normalized, message_text)
            if success:
                sms_sent = True
                logger.info(f"SMS OTP sent to {phone} msgid={resp}")
            else:
                logger.warning(f"SMS send failed: {resp}")
        except Exception:
...
def login():
    """Login with email and password, validating checks and establishing session."""
    ...
    # Hydrate Server-Side Session Mapping
...
def verify_email():
    ...
    # Activate the plan chosen during signup flow
    chosen_plan = (data.get("plan") or "").strip().lower()
    if chosen_plan and chosen_plan in ("beta", "starter", "growth", "enterprise"):
        user.plan = chosen_plan
        if chosen_plan == "beta":
            user.beta_expires_at = datetime.utcnow() + timedelta(days=30)
        else:
            user.beta_expires_at = None
    elif not user.plan or user.plan == "pending_verification":
        user.plan = "beta"
        user.beta_expires_at = datetime.utcnow() + timedelta(days=30)
...
def resend_code():
    ...
    try:
        ...
        send_mail(user.email, "Verify your SocioChat account", email_body)
        email_sent = True
    except Exception:
        logger.exception("Failed to send verification email on resend")

    # 2. Also send via SMS if user has a phone
    if user.phone:
        try:
            from sms_routes import send_sms_horizon
            message_text = (
                " Dear Customer,\n"
                f"Your One-Time Password (OTP) is {verification_code}.\n"
                "Please do not share this code with anyone for security reasons.\n\n"
            ...
            success, resp = send_sms_horizon(user.phone, message_text)
            if success:
                sms_sent = True
        except Exception:
...
def forgot_password():
    ...
    try:
        ...
        send_mail(user.email, "SocioChat — Password reset instructions", email_body)
...
def reset_password():
    ...
    try:
        ...
        send_mail(user.email, "Your SocioChat password has been changed", email_body)
